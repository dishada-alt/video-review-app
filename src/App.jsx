...
const ALLOWED_METADATA_COLUMNS = [
  'Ad ID',
  'Campaign Concept',
  'Impressions',
  'Clicks',
...
export default function App() {
  ...
  const sortableColumns = useMemo(() => {
    ...
    return ALLOWED_METADATA_COLUMNS.filter((key) => key in rows[0]);
  }, [rows]);
  ...
  const filteredAndSortedRows = useMemo(() => {
    ...
    if (term) {
      filtered = rows.filter((row) =>
        ['video_url', ...ALLOWED_METADATA_COLUMNS].some((column) =>
    ...
    if (sortColumn) {
      const sortIsNumeric = isNumericColumn(rows, sortColumn);
      ...
      filtered = [...filtered].sort((a, b) => {
        ...
        if (sortIsNumeric) {
          const aValue = Number(rawA);
          const bValue = Number(rawB);
  ...
  const metadataColumnsForRow = (row) =>
    ALLOWED_METADATA_COLUMNS.filter((column) => Object.prototype.hasOwnProperty.call(row, column));
    ...
    <main className="container">
      ...
      <section className="cards-grid">
        {visibleRows.map((row) => {
          ...
          const metadataColumns = metadataColumnsForRow(row);
